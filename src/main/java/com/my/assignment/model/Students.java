package com.my.assignment.model;
...
public class Students {
    ...
    public void filterListByName(String name){ //, String email) {
    ...
    public void filterListByEmail(String email){ //, String email) {

        List<Student> temp = new ArrayList<>();

        students.forEach((Student p) -> {
            try {
                if ((p.hasEmail(email))){ //|| (p.hasEmail(email))) {
                    temp.add(p);
                }
            } catch (NumberFormatException e) {
            }
